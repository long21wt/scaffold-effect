"""
Evaluate MDD classification results from JSONL inference output files.
Computes accuracy, precision, recall, and F1 for Control vs. MDD predictions.
...
def process_file(filepath, true_label, force_extract=False):
    ...
    for entry in tqdm(data, desc=f"Processing {os.path.basename(filepath)}"):
        ...
        if not force_extract and "extracted_label" in entry:
            ...
            if '"category": "Major Depressive Disorder' in output or '"Major Depressive Disorder' in output or "{'category': 'Major Depressive Disorder" in output:
                predicted_label = "MDD"
            elif '"category": "Control' in output or '"Control (no disorder detected)' in output or "{'category': 'Control" in output:
...
def parse_args():
    ...
    p.add_argument("--mdd_file", required=True,
                   help="Path to JSONL file with MDD predictions")
...
if __name__ == "__main__":
    ...
    control_results = process_file(args.control_file, "Control",
                                   force_extract=args.force_extract)

    print("\n" + "="*50)
    print("Processing MDD samples...")
    mdd_results = process_file(args.mdd_file, "MDD",
                               force_extract=args.force_extract)
