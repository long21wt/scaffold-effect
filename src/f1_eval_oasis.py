"""
Evaluate cognitive-decline classification results from JSONL inference output files (OASIS dataset).
Computes accuracy, precision, recall, and F1 for Cognitive Normal vs. Cognitive Decline predictions.
...
def process_file(filepath, true_label, force_extract=False):
    ...
    for entry in tqdm(data, desc=f"Processing {os.path.basename(filepath)}"):
        ...
        if not force_extract and "extracted_label" in entry:
            ...
            print(f"Using existing label for {entry.get('filename', 'Unknown')}: {predicted_label}")
        else:
            output = entry["output"]
            # Determine predicted label
            if '"category": "Cognitive Decline' in output or '"Cognitive Decline' in output or "{'category': 'Cognitive Decline" in output:
                predicted_label = "Cognitive Decline"
            elif '"category": "Cognitive Normal' in output or '"Cognitive Normal' in output or "{'category': 'Cognitive Normal" in output:
                ...
                if extracted_response == "1":
                    ...
                    predicted_label = "Cognitive Decline" if true_label == "Cognitive Normal" else "Cognitive Normal"
                modified = True
...
def calculate_metrics(all_results):
    ...
    for true_label, predicted_label in all_results:
        if true_label == "Cognitive Decline" and predicted_label == "Cognitive Decline":
            ...
            tn += 1
        elif true_label == "Cognitive Normal" and predicted_label == "Cognitive Decline":
            fp += 1
        elif true_label == "Cognitive Decline" and predicted_label == "Cognitive Normal":
            fn += 1
...
if __name__ == "__main__":
    ...
    control_results = process_file(args.cn_file, "Cognitive Normal",
                                   force_extract=args.force_extract)

    print("\n" + "="*50)
    print("Processing Cognitive Decline samples...")
    mdd_results = process_file(args.cd_file, "Cognitive Decline",
                               force_extract=args.force_extract)
    ...
    print("="*50)
    print(f"Accuracy:    {metrics['accuracy']:.4f} ({metrics['tp'] + metrics['tn']}/{metrics['total']})")
