"""
Finds token sequences that activate the same "scaffold direction" as the MRI preamble - i.e. sequences that shift the layer-33 hidden state in the direction that causes the final layers to route toward MDD.

# E.g., full pipeline, ~20 min on 30 patients):
...
class ScaffoldModel:
    ...
    def _prepare(self, messages):
        ...
        return self.processor(
            text=[tmpl], images=img_in, videos=vid_in,
    ...
    def get_label_hidden_state(
        ...
        for i, tid in enumerate(new_ids):
            if tid == self.mdd_tok_id:
                label_step  = i; found_label = "MDD"; break
            if tid == self.ctrl_tok_id:
                label_step  = i; found_label = "Control"; break
        ...
        with torch.inference_mode():
            logits = self.lm_head(self.final_ln(h_final).to(
                self.lm_head.weight.device)).float()[0]
...
def load_patients(txt_path, label, n=None):
    ...
    for f in sorted(os.listdir(txt_path)):
        ...
        with open(os.path.join(txt_path, f)) as fh: text = fh.read()
...
def extract_scaffold_direction(
    ...
    print(f"  Patients: {len(patients)}  ×  2 conditions = {2*len(patients)} forward passes")
    ...
    pairs = [(ht, hm) for ht, hm in zip(h_tabular, h_mri)
    ...
    print(f"  Mean cos(h_mri, dir): "
          f"{np.dot(h_mri_arr / (np.linalg.norm(h_mri_arr, axis=1, keepdims=True) + 1e-9),
                    scaffold_dir_norm).mean():.4f}")
...
def single_token_search(
    ...
    if res_base is None:
        print("  [ERR] Baseline failed"); return []
    h_base = res_base[0]
...
def phrase_search(
    ...
    for category, phrases in CANDIDATE_PHRASES.items():
        ...
        for phrase in phrases:
            ...
            for pid, text, true_label in patients:
                if pid not in baselines_h: continue
                ...
                cos_sim = float(np.dot(shift / (np.linalg.norm(shift) + 1e-9),
                                       scaffold_dir))
                cos_sims.append(cos_sim)
                p_mdds.append(p_mdd)
                p_shifts.append(p_mdd - baselines_pm[pid])

            if not cos_sims: continue
            ...
            print(f"    cos={result['cos_sim_mean']:+.3f}  "
    ...
    for i, r in enumerate(all_results):
        print(f"  {i+1:<4} {r['cos_sim_mean']:>+8.4f} {r['p_mdd_mean']:>8.4f} "
...
def plot_phrase_results(all_results: List[Dict], output_dir: str):
    ...
    for ax, vals, ylabel, title in [
        (axes[0], cat_cos,   "Cosine sim to scaffold direction",
         "Alignment with scaffold direction\n(by phrase category)"),
        (axes[1], cat_shift, "ΔP(MDD) vs tabular baseline",
...
def plot_top_phrases_scatter(all_results, output_dir):
    ...
    for r in all_results:
        ax.scatter(r["cos_sim_mean"], r["p_shift_mean"],
                   color=cdict[r["category"]], s=60, alpha=0.8, zorder=3)
        if abs(r["cos_sim_mean"]) > 0.1 or abs(r["p_shift_mean"]) > 0.05:
            ax.annotate(r["phrase"][:35], (r["cos_sim_mean"], r["p_shift_mean"]),
    ...
    ax.set_title("Equivalence class of scaffold-activating phrases — FOR2107\n"
                 "Top-right quadrant = same effect as MRI preamble", fontsize=9)
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    if not args.skip_token_search:
        top_tokens = single_token_search(
            model, scaffold_dir, mdd_pts[:3],   # 3 patients for speed
            args.output_dir, args.scaffold_layer, vocab_sample=args.vocab_sample)
    else:
        print("\nSkipping Step 2 (single-token search)")

    phrase_results = phrase_search(
        model, scaffold_dir, mdd_pts,
        args.output_dir, args.scaffold_layer)
